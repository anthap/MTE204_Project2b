...
%     elseif solveMethod == 2
        solveImplicit(M,C,K,F,U,timeStep,endTime,nodeToEvaluate);
%     end
end
...
function solveImplicit(M,C,K,F,U,timeStep,endTime,nodeToEvaluate);
    starttime = cputime;
...
%     Fsolve = zeros(numNodes*2,1);
    for i = 1:size(Uorganize,1)
        Usolve(order(i)) = UorganizeCurr(i);
%         Fsolve(orderF(i)) = ForganizeCurr(i);
    end
